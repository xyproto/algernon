...
func generateUsageFunction(ac *algernonConfig) func() {
	return func() {
		fmt.Println("\n" + versionString + "\n\n" + description)
...
func (ac *algernonConfig) handleFlags(serverTempDir string) {
	var (
	...
	flag.StringVar(&ac.serverHeaderName, "servername", versionString, "Server header name")
	...
	if ac.productionMode {
		...
		ac.cacheMode = cacheModeDevelopment
	} else if ac.simpleMode {
		ac.useBolt = true
		ac.boltFilename = "/dev/null"
		ac.serveJustHTTP = true
		ac.serverMode = true
		ac.cacheMode = cacheModeOff
	...
	if cacheModeString != "" {
		ac.cacheMode = newCacheModeSetting(cacheModeString)
	}
	...
	if ac.cacheMode == cacheModeOff {
		ac.cacheSize = 0
	}

	// If cache mode is unset, use the dev mode
	if ac.cacheMode == cacheModeUnset {
		ac.cacheMode = cacheModeDefault
	}

	if ac.cacheMode == cacheModeSmall {
	...
	if ac.openURLAfterServing && ac.openExecutable == "" {
		ac.openExecutable = defaultOpenExecutable
	}
...
func (ac *algernonConfig) finalConfiguration(host string) bool {
