...
import (
	...
	// pure-Go MIT-compatible SQLite driver (via wazero)
...
func (l *List) LastUpToN(n uint64) ([]string, error) {
	rows, err := l.file.db.Query("SELECT " + listCol + " FROM (SELECT * FROM " + l.table + " ORDER BY id DESC LIMIT " + strconv.FormatUint(n, 10) + ") sub ORDER BY id ASC")
	if err != nil {
		return []string{}, err
	}
	defer rows.Close()
