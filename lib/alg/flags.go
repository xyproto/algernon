...
func (ac *Config) handleFlags(serverTempDir string) {
	...
	if ac.productionMode {
		...
		ac.cacheMode = cache.ModeProduction
		ac.serverMode = true
	} else if ac.devMode {
		...
		ac.cacheMode = cache.ModeDevelopment
	} else if ac.simpleMode {
		ac.useBolt = true
		ac.boltFilename = "/dev/null"
		ac.serveJustHTTP = true
		ac.serverMode = true
		ac.cacheMode = cache.ModeOff
	...
	if cacheModeString != "" {
		ac.cacheMode = cache.NewModeSetting(cacheModeString)
	}
	...
	if ac.cacheMode == cache.ModeOff {
		ac.cacheSize = 0
	}

	// If cache mode is unset, use the dev mode
	if ac.cacheMode == cache.ModeUnset {
		ac.cacheMode = cache.ModeDefault
	}

	if ac.cacheMode == cache.ModeSmall {
